...
修复位姿丢失恢复：PlayCanvas 在没有头部位姿时跳过 XR update，因此仅在普通 update 内检查时间不足以发现恢复前的中断。现在在新有效帧覆盖时间戳前检查超过 250 ms 的间隔，清除旧手势、等待摇杆回中；事件帧无头部位姿时也不能提交选择。行为测试覆盖空头部位姿、恢复时仍推住摇杆、回中重新启用，以及被引擎跳过的帧间隔。
