...
change_id: MF-1
tool: codex
source_task_id: 019fe7de-0291-7362-a282-8e9fabc2ef1f
status: complete
started: 2026-08-09T18:51:54Z
completed: 2026-08-09T18:54:12Z
branch: codex/mcl-v1
...
implementer_id: codex/019fe7de-0291-7362-a282-8e9fabc2ef1f
review_relationship: author-self-review
reviewer_id: codex/019fe7de-0291-7362-a282-8e9fabc2ef1f
...
Remove versioned Supersplat snapshots from Dependabot coverage, add a platform-policy regression check, and restore exactly the two `supersplat-v2.28.0` package metadata files to the PR base. The Task did not authorize restoring the whole upstream tree, moving Editor code, changing product behavior, disposing of PRs #4–#8, publishing Git changes, or changing external state.
...
- Scope of authority: Dependabot configuration, platform validation/tests, and exact restoration of two package files.
- Review relationship: the implementer inspected and tested its own focused diff; no independent non-author Review is claimed.
...
The component registry was intentionally not modified because path ownership already represents CI routing; the document Task supplied the non-authorization clarification without changing the registry schema. No other deviation occurred.

## Remaining Work and Continuation Conditions

The primary Agent must verify that the two package files disappear from the final PR diff against `origin/main`, integrate the platform checks into full validation, and leave PR #4–#8 disposition outside MF-1.

## Final Delivery

The uncommitted dependency-boundary changes and focused validation evidence were returned to the primary MF-1 Agent in the shared worktree.
