...
completed: 2026-08-14T10:29:15Z
branch: codex/mf-30-viewer-5.19.1-release-record
head_before: 8a5857ebec23a30f6d9eabd5050eaed55d911ce6
head_after: f0fb740dd77d67e4f1be781934ce7a4fa095c30a
plan_revision: 3
plan_sha256: 33098b04239ef1965a391c0ed957512afc51ce05fe6bb6ed1c4a41c8abdc3482
...
A sparse release worktree was used to avoid duplicating large data on the constrained Prism volume; full-data validation uses existing read-only fixtures. This preserves isolation and test identity. Revision 3 uses PATCH `5.19.1`, not `5.20.0`, because the recovery changes only validation/deployment compatibility. It preserves the failed `5.19.0` Tag/run, the one-time migration marker `5.19.0`, all Viewer runtime contracts, and all 87 route selections. The plan expected the recovery merge's ordinary-main deploy to become `skipped`; it is presently queued `new`, so the continuation gate remains deliberately closed rather than silently treating a queued record as success.

## Remaining Work and Continuation Conditions

Obtain a clear non-publishing outcome for ordinary-main recovery record `6a7ee320ef4dbd0008dd089f` without touching excluded PR/branch previews. Complete and strictly validate D2; re-read stable/main/production; fast-forward R2+D2 to `origin/main`; confirm that ordinary Git integration cannot publish; create immutable `viewer-v5.19.1`; run and verify the controlled workflow/deploy; perform immediate production smoke and 15-minute observation; then create E2, update the GitHub Release and both merged PRs, close Issue #30, read back every external surface, and remove only the approved clean worktrees. Roll back to deploy `6a7a18b49094c6c76eff2482` if production fails after publication.

## Final Delivery

This task remains partial at the `5.19.1` ready-for-release recovery boundary. Production remains Viewer `5.18.1`. Final delivery will be added only after ordinary-main gating, the authorized controlled release, immediate smoke, observation, final external readback, and Issue closure complete.
