...
# MF-30 Viewer 5.19.0 formal release proposal

## Decision

Release the already-reviewed and merged Metaflow Viewer `5.19.0` product commit `26e311c010aea4a6202521453a034d5aef3cea54`, based on SuperSplat Viewer `v1.29.1` commit `3a61fa606e12640b1e87f9a733ed43d7fbc5d925`, tree `671059c4b4a3693115ad010207b66312f5cbbc8c`, and PlayCanvas `2.21.3`.

The user explicitly authorized the squash merge and full formal release on 2026-08-14, including merge-SHA reconciliation, an immutable `viewer-v5.19.0` Tag, the controlled GitHub Actions to Netlify production path, production smoke, a 15-minute observation, final audit records, and Issue closure. Production remains `5.18.1` until that entire sequence succeeds.

## Why release

The upstream line adds and corrects on-demand rendering, near-clip handling, streamed work-buffer ordering, capture, Annotation preference, WebGPU/WebXR detection, preference lifecycle, programmable locale selection, and SH update cadence. The Metaflow implementation preserves the local product layer instead of replacing it: route/index/alias contracts, legacy SOG and streamed LOD, non-blocking environment, first-frame/reveal sequencing, Orbit/walk/fly/mobile controls, single/tiled voxel collision, `metaflow-rz180`, settings v1/v2, brand, nine locales, Analytics, XR navigation, and diagnostics.
...
- The production smoke and 15-minute observation must both pass before `5.19.0` is recorded as stable and Issue #30 is closed.
...
If the new deploy is published but smoke or observation fails, invoke the repository rollback workflow with that exact deploy ID, verify production recovery, keep MF-30 open, and append the failure evidence. The pushed `viewer-v5.19.0` Tag is immutable. Any post-Tag product correction requires a separate fix and `5.19.1`.
