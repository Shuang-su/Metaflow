...
| 变化 | 版本处理 | 示例 |
|---|---|---|
| 资源、thumbnail、settings、兼容 route/alias、Bug、小型兼容行为或部署修复 | PATCH | `5.19.2` |
...
当前生产运行 SuperSplat Viewer `v1.29.1` / PlayCanvas `2.21.3`，运行时产品 SHA 为 `26e311c`，5.19.2 analytics/release 修复 SHA 为 `92d11b0`，display/package 为 `5.19.2`。`viewer-v5.19.0` 已建立但 prepare 在 deployment 前失败，没有 GitHub Release，生产从未切换；`viewer-v5.19.1` 已在 D2 controlled Prepare 后通过经授权的精确 D2 CLI/API fallback 发布，`5.19.2` 修复其漏注入的 Supabase endpoint，并完成 production smoke。`5.18a` 及更早字母版本保持不变；`5.18.1` 是首个后续完整 SemVer 发布，之后不再创建 `5.18b`。
