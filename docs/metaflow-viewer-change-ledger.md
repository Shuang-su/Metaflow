...
本文档完整审计 `main` 从初始提交到历史边界 `c613a87` 的产品与维护提交；从该边界之后只审计 Viewer、data 和 Viewer 发布支撑提交。它不是全仓提交标题的重排，而是用于回答：
...
纯文档、MCL/治理、Design、Reference、Editor 和无关平台维护既不形成 Viewer 产品版本，也不进入 Viewer 总账。功能、修复、公开资源和实际部署提交必须同时更新结构化历史与本总账；staging、版本修正和发布支撑提交可以进入 `maintenanceCommits`。

`documentedThrough` 表示 Viewer 行为审计边界，不表示仓库所有组件的提交都已进入本总账。历史边界之前的既有记录保持原样，不按新范围回写或删除。
...
| `5.19.1` · `534b013` | `viewer-v5.19.0` 已指向不可变 release packet `f1986097`，但 controlled workflow [`31779246997`](https://github.com/Shuang-su/Metaflow/actions/runs/31779246997) 在 prepare 阶段只得到 Viewer `81/85`：release sparse checkout 缺根 `.nvmrc`、BitCity/SZCAF15 fixture，并且 package consumer tests 在 `dist/` build 前运行。production job 被跳过，没有 GitHub Release，`5.19.0` 在 deployment 前失败且生产一直是 `5.18.1`。同时普通 main Git push 仍会创建可发布的 production build，和“只有 Tag workflow 可发布”的控制目标不一致。 | Recovery PR #45 补齐 release 与 on-demand CI 的 `.nvmrc`、BitCity、SZCAF15 sparse fixture，把 production build 移到 85 项测试前；新增 `viewer-vX.Y.Z` 精确解析并同时核对 package/lock、metadata/public current、index release 与产品 `gitRef`；production smoke 对 immutable/production index 使用精确版本和 gitRef，而非 truthy 检查；Netlify selected deploy 还必须同时匹配 Tag target commit、Tag title 和 production context。`netlify.toml` 让普通 `main` Git build 通过 ignore 跳过，PR/feature Preview 保留，受控 build hook 继续作为唯一 production 入口。Viewer 运行时源码、偏好迁移标记 `5.19.0`、87 条 route、9 条 streaming、78 条 SOG、schema 与资源 payload 均未改变。 | 发布修复使用 PATCH `5.19.1`，机器/public 版本面指向 recovery squash SHA `534b013`；用户实际升级路径仍是生产 `5.18.1 → 5.19.1`，不会先收到 5.19.0。新的 `viewer-v5.19.1` Tag、受控部署、production smoke 与 15 分钟观察完成前，正式生产发布仍在等待。 | 风险集中在 Netlify ignore/build-hook 队列和精确 deploy 选择，而不是 Viewer 产品行为。全新 exact-sparse checkout 已完成 build 后 Viewer `85/85`、fmt、lint、typecheck、publint 与 0 production vulnerabilities；GitHub 首次 on-demand run `31788608476` 精确复现旧 CI 的同四项失败，修正后的 run [`31788879015`](https://github.com/Shuang-su/Metaflow/actions/runs/31788879015) 通过治理与 CodeQL，但按路径路由跳过 Viewer，因此不冒充云端 85/85。PR #45 无独立 reviewer；移动真机与 immersive XR 限制继承 5.19.0。回退 production deploy 固定为 `6a7a18b49094c6c76eff2482`；`viewer-v5.19.0` 永不移动或删除。 |

`5.19.0` 失败尝试的不可变身份为 Tag object `c9a19ea438e604333af2d3158bebea7d16f1a33e`、peeled target / release packet `f1986097f81cf15db95d33fa76c090b2066d4bd1`；这些证据只追加，不删除、不移动，也不创建 `viewer-v5.19.0` GitHub Release。
