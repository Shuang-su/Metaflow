...
fn queryBlock(bx: i32, by: i32, bz: i32) -> vec2u {
    ...
    for (var level: u32 = depth - 1u; ; ) {
        let nodePtr = nodeIndex * uniforms.nodeStride;
        let node = nodes[nodePtr];

        // Solid leaf sentinel -- return level + 1 so the caller can skip 2^(level+1) blocks
        if ((uniforms.nodeStride == 1u && node == SOLID_LEAF_MARKER) ||
            (uniforms.nodeStride == 2u && node == 0xFFu)) {
            return vec2u(1u, level + 1u);
        }

        let childMask = select((node >> 24u) & 0xFFu, node & 0xFFu, uniforms.nodeStride == 2u);
        let baseOffset = select(node & 0x00FFFFFFu, nodes[nodePtr + 1u], uniforms.nodeStride == 2u);

        // childMask == 0 means this is a mixed leaf node
        if (childMask == 0u) {
            let leafIdx = baseOffset;
    ...
    let leafIdx = select(node & 0x00FFFFFFu, nodes[nodePtr + 1u], uniforms.nodeStride == 2u);
    return vec2u(2u + leafIdx, 0u);
}
...
class VoxelDebugOverlay {
        ...
        const shaderDefinition = {
            ...
            computeUniformBufferFormats: {
                uniforms: new UniformBufferFormat(device, [
                    ...
                    new UniformFormat('nodeStride', UNIFORMTYPE_UINT)
                ])
            },
    ...
    update(): void {
        ...
        compute.setParameter('nodeStride', collision.nodeStride);
...
class TiledVoxelDebugOverlay {
    private readonly overlays = new Map<VoxelCollision, VoxelDebugOverlay>();
    ...
    update(): void {
        const active = new Set(this.collision.getActiveColliders());
        for (const [collision, overlay] of Array.from(this.overlays.entries())) {
        ...
        for (const collision of active) {
            if (!this.overlays.has(collision)) {
                this.overlays.set(collision, new VoxelDebugOverlay(this.app, collision, this.camera));
            }
        }
