...
import { css as legacyViewerCss, html as legacyViewerHtml, js as legacyViewerJs } from '@playcanvas/supersplat-viewer';
import {
    GSplatData,
    Mat3,
    Mat4,
...
import { metaflowEditorLabel, upstreamEditorLabel } from './metaflow-editor-version';
...
type ViewerExportSettings = {
    type: 'html' | 'zip' | 'legacyZip' | 'settingsJson';
...
const generatedByString = `Generated by ${metaflowEditorLabel} (${upstreamEditorLabel})`;
...
const writeFile = async (fs: FileSystem, filename: string, data: Uint8Array | string): Promise<void> => {
    const writer = await fs.createWriter(filename);
    try {
        await writer.write(typeof data === 'string' ? textEncoder.encode(data) : data);
    } finally {
        await writer.close();
    }
};

const writeZipFile = async (zipFs: ZipFileSystem, filename: string, data: Uint8Array | string): Promise<void> => {
...
const writeViewerSettings = async (experienceSettings: ExperienceSettings, fs: FileSystem): Promise<void> => {
    await writeFile(fs, 'settings.json', `${JSON.stringify(experienceSettings, null, 2)}\n`);
};

const serializeLegacyViewer = async (splats: Splat[], serializeSettings: SerializeSettings, experienceSettings: ExperienceSettings, fs: FileSystem): Promise<void> => {
    const plyFs = new MemoryFileSystem();
    await serializePlyCompressed(splats, {
        ...serializeSettings,
        minOpacity: 1 / 255,
        removeInvalid: true
    }, plyFs);

    const compressedPly = plyFs.results.get('output.compressed.ply');
    if (!compressedPly) {
        throw new Error('Failed to generate scene.compressed.ply');
    }

    const zipWriter = await fs.createWriter('output.zip');
    const zipFs = new ZipFileSystem(zipWriter);
    try {
        await writeZipFile(zipFs, 'index.html', legacyViewerHtml);
        await writeZipFile(zipFs, 'index.css', legacyViewerCss);
        await writeZipFile(zipFs, 'index.js', legacyViewerJs);
        await writeZipFile(zipFs, 'settings.json', `${JSON.stringify(experienceSettings, null, 2)}\n`);
        await writeZipFile(zipFs, 'scene.compressed.ply', compressedPly);
    } finally {
        await zipFs.close();
    }
};
...
const serializeViewer = async (splats: Splat[], serializeSettings: SerializeSettings, options: ViewerExportSettings, fs: FileSystem): Promise<void> => {
    ...
    if (options.type === 'settingsJson') {
        await writeViewerSettings(experienceSettings, fs);
        return;
    }

    if (options.type === 'legacyZip') {
        await serializeLegacyViewer(splats, serializeSettings, experienceSettings, fs);
        return;
    }
